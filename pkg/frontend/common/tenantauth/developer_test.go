...
func testJWT(t *testing.T, sub, role string) string {
	t.Helper()
...
func TestAuthenticateDeveloperTokenCachesSuccessfulIAMValidation(t *testing.T) {
	...
	token := testJWT(t, "user1", jwtauth.RoleDeveloper)
	for i := 0; i < 2; i++ {
...
func TestAuthenticateDeveloperTokenRejectsNonDeveloperRole(t *testing.T) {
	...
	_, status, err := AuthenticateDeveloperToken(testJWT(t, "user1", jwtauth.RoleUser), "trace")
	if err == nil {
