...
func TestValidateWithIamServer_CacheExpiry(t *testing.T) {
	setupIamTest(t)
	useIAMValidationCacheForTest(t, 32, 10*time.Millisecond)
	var callCount atomic.Int32
	origValidator := iamValidator
	iamValidator = func(token, traceID string) error {
		callCount.Add(1)
...
func TestValidateWithIamServer_LRUEvictsLeastRecentlyUsed(t *testing.T) {
	setupIamTest(t)
	useIAMValidationCacheForTest(t, 2, time.Minute)
	var callCount atomic.Int32
	origValidator := iamValidator
	iamValidator = func(token, traceID string) error {
		callCount.Add(1)
