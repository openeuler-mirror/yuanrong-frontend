...
func TestSandboxDirectRouteForwardsToLocalSandboxRouter(t *testing.T) {
	...
	listener, backend, seen := startSandboxDirectBackend(t)
	defer listener.Close()
	defer backend.Close()

	initSandboxDirectConfig(listener)
	...
	req, err := http.NewRequest(
		http.MethodPost,
		server.URL+"/direct/demo/invoke?x=1&token=secret&tenant_id=default",
		nil,
	)
	if err != nil {
	...
	req.Header.Set(constant.CaaSHeaderTraceID, "trace-direct")
	...
	assertSandboxDirectForwarding(t, <-seen, resp)
}

func assertSandboxDirectForwarding(t *testing.T, got *http.Request, resp *http.Response) {
	t.Helper()
	...
	if got.Header.Get(constant.HeaderTraceID) != "trace-direct" {
		t.Fatalf("trace header was not forwarded: %q", got.Header.Get(constant.HeaderTraceID))
	}
	if resp.Header.Get(constant.HeaderTraceID) != "trace-direct" {
		t.Fatalf("trace header was not returned: %q", resp.Header.Get(constant.HeaderTraceID))
	}
}

func startSandboxDirectBackend(t *testing.T) (net.Listener, *http.Server, chan *http.Request) {
	t.Helper()
	...
	go func() {
		if err := backend.Serve(listener); err != nil && err != http.ErrServerClosed {
			t.Errorf("serve backend: %v", err)
		}
	}()
	return listener, backend, seen
}

func initSandboxDirectConfig(listener net.Listener) {
	initSandboxDirectConfigWithAuth(listener, true)
}

func initSandboxDirectConfigWithAuth(listener net.Listener, authEnabled bool) {
	config.InitFunctionConfig([]byte(fmt.Sprintf(`{
		...
		"trafficLimitDisable":true,
		"businessType":1,
		"iamConfig":{"enableFuncTokenAuth":%t},
		"sandboxRouter":{"enabled":true,"listenIP":"127.0.0.1","listenPort":%d}
	}`, authEnabled, listener.Addr().(*net.TCPAddr).Port)))
}
...
func TestSandboxDirectRouteDefersAuthToRouterWhenFrontendJWTDisabled(t *testing.T) {
	...
	initSandboxDirectConfigWithAuth(listener, false)
