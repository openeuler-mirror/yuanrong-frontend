...
func authorizedTokenProxyJWT(t *testing.T) string {
...
func TestRequireTokenProxyForwardsDeveloperTokenRequest(t *testing.T) {
	...
	ctx.Request.Header.Set(iamHeaderAuth, authorizedTokenProxyJWT(t))
...
func TestRequireTokenProxyDelegatesCallerValidationEachRequest(t *testing.T) {
	...
	if validateCalls != 2 {
		t.Fatalf("validated caller token %d times, want 2", validateCalls)
	}
	if client.requireCalls != 2 {
