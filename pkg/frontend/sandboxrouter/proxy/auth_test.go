...
const (
	proxyTestRRTPort       = 50090
	proxyTestTunnelWSPort  = 8765
	proxyTestUserHTTPPort  = 8080
	proxyTestOffLoopbackIP = "10.0.0.8:43210"
)

func writeString(t testing.TB, w http.ResponseWriter, body string) {
	t.Helper()
...
func mintJWT(t testing.TB, sub string, exp int64) string {
	t.Helper()
	...
	payload := enc(jwtauth.JWTPayload{Sub: sub, Exp: exp, Role: jwtauth.RoleDeveloper})
...
func TestAuthExpiredTokenIs401(t *testing.T) {
	s := New(fakeResolver{target: targetTo(t, "http://127.0.0.1:1")})
	s.SetAuth(true, false, proxyTestRRTPort, proxyTestTunnelWSPort)
	rec := doAuth(s, http.MethodGet, "/default-rtt/50090/healthz", mintJWT(t, "default", 1))
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("status = %d, want 401", rec.Code)
	}
}
...
func TestAuthTenantMismatchIs403(t *testing.T) {
	...
	s.SetAuth(true, false, proxyTestRRTPort, proxyTestTunnelWSPort)
	rec := doAuth(s, http.MethodGet, "/default-rtt/50090/healthz", mintJWT(t, "tenant-b", farFuture))
...
func TestAuthValidTokenForwardedToRRTPort(t *testing.T) {
	...
	s := New(fakeResolver{target: tg})
	s.SetAuth(true, false, proxyTestRRTPort, proxyTestTunnelWSPort)
	token := mintJWT(t, "default", farFuture)
...
func TestSpoofedFrontendInternalHeaderOffLoopbackStillRequiresToken(t *testing.T) {
	...
	req.RemoteAddr = proxyTestOffLoopbackIP
	req.Header.Set(internalSrcHeader, internalSrcAuthenticatedValue)
	rec := httptest.NewRecorder()
	s.ServeHTTP(rec, req)
...
func TestAuthTokenStrippedForUserPort(t *testing.T) {
	...
	s := New(fakeResolver{target: tg})
	s.SetAuth(true, false, proxyTestRRTPort, proxyTestTunnelWSPort)
	rec := doAuth(s, http.MethodGet, "/default-app/8080/api", mintJWT(t, "default", farFuture))
...
func TestAuthTunnelPortValidTokenOK(t *testing.T) {
	...
	s := New(fakeResolver{target: tg})
	s.SetAuth(true, false, proxyTestRRTPort, proxyTestTunnelWSPort)
	rec := doAuth(s, http.MethodGet, "/default-rtt/8765/healthz", mintJWT(t, "default", farFuture))
	if rec.Code != http.StatusOK {
