...
func getRoute(c *Cache) (*Target, error) {
	return c.Get(Key{SafeInstanceID: "inst-abc", Port: 8765})
}
...
func TestApplyRunningWithoutPortForwardAddsNothing(t *testing.T) {
	c := NewCache()
	body := `{"instanceID":"inst-abc","tenantID":"tenant-a","proxyGrpcAddress":"10.0.0.1:22772",` +
		`"instanceStatus":{"code":3},"extensions":{}}`
