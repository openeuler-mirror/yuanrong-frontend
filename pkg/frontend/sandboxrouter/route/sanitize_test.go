...
const maxSanitizedIDLength = 200

// TestSanitizeID locks the cross-language contract with the C++
// TraefikCache::SanitizeID (functionsystem/.../traefik_route_cache.cpp).
...
func TestSanitizeID(t *testing.T) {
	cases := []struct {
		...
		// @ is expanded to -at- BEFORE truncation.
		{"truncate after expansion", strings.Repeat("@", 100), strings.Repeat("-at-", maxSanitizedIDLength/4)},
		// Plain chars truncate to exactly maxSanitizedIDLength.
		{"truncate plain", strings.Repeat("a", 250), strings.Repeat("a", maxSanitizedIDLength)},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if len(got) > maxSanitizedIDLength {
				t.Errorf("SanitizeID(%q) length = %d, must be <= %d", c.in, len(got), maxSanitizedIDLength)
			}
		})
	}
}

// TestExtractIP locks the contract with C++ TraefikCache::ExtractIP:
